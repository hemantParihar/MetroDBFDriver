...
public class DBFPreparedStatement extends DBFStatement implements PreparedStatement {
    ...
    public DBFPreparedStatement(DBFConnection connection, String sql) throws SQLException {
        ...
        // Parse SQL once and cache the AST. Only SELECT statements have a
        // parseable AST; DML/DDL statements are dispatched as strings.
        this.parsedSql = extractParameterPlaceholders(sql);
        if (sql.trim().toUpperCase().startsWith("SELECT")) {
            this.cachedSelectNode = parseAndCache(parsedSql);
        } else {
            this.cachedSelectNode = null;
        }
    ...
    private int estimateParameterCount(String sql) {
        ...
        // Exactly the number of '?' placeholders - a parameterless query has
        // zero, and must not be forced to expect one.
        return count;
    }
    ...
    public int[] executeBatch() throws SQLException {
        ...
        // INSERT batches go through a single writer (one file open, one
        // header rewrite) instead of executing row by row
        if (originalSql.trim().toUpperCase().startsWith("INSERT")) {
            int[] results = executeInsertBatch();
            clearBatch();
            return results;
        }
    ...
    /**
     * Fast path for batched INSERTs: the statement shell (table, columns,
     * value template) is parsed once, then every parameter set is bound
     * and written through one DBFWriter.
     */
    private int[] executeInsertBatch() throws SQLException {
        ...
        if (intoPos < 4 || valuesPos < 0) {
            throw new SQLException("Malformed INSERT statement: " + originalSql);
        }

        String beforeValues = originalSql.substring(intoPos, valuesPos).trim();
        String tableName;
        String[] columns = null;
        int colParen = beforeValues.indexOf('(');
        if (colParen >= 0) {
            tableName = beforeValues.substring(0, colParen).trim();
            String columnsStr = beforeValues.substring(colParen + 1, beforeValues.lastIndexOf(')'));
            columns = columnsStr.split(",");
            for (int i = 0; i < columns.length; i++) {
                columns[i] = columns[i].trim();
            }
        } else {
            tableName = beforeValues;
        }

        int valuesStart = originalSql.indexOf('(', valuesPos);
        int valuesEnd = originalSql.lastIndexOf(')');
        if (valuesStart < 0 || valuesEnd <= valuesStart) {
            throw new SQLException("Malformed VALUES clause: " + originalSql);
        }
        List<String> valueTemplate = splitCsvRespectingQuotes(
            originalSql.substring(valuesStart + 1, valuesEnd));

        // Bind each parameter set against the template
        List<Object[]> rows = new ArrayList<>(batchParameters.size());
        for (Object[] params : batchParameters) {
            Object[] row = new Object[valueTemplate.size()];
            int paramIndex = 0;
            for (int i = 0; i < valueTemplate.size(); i++) {
                String token = valueTemplate.get(i).trim();
                if ("?".equals(token)) {
                    if (paramIndex >= params.length) {
                        throw new SQLException("Not enough parameters bound for batch row");
                    }
                    row[i] = params[paramIndex++];
                } else {
                    row[i] = parseValue(token);
                }
            }
            rows.add(row);
        }
        ...
        try {
            ...
            // Maintain indexes once for the whole batch (all new RECNOs are in
            // lastGeneratedKeys) rather than per row.
            maintainIndexesAfterInsert(tableName);
            return results;
        } catch (IOException e) {
            throw writeError("Batch insert", e);
        }
    }
