...
public class SortEngine {
    private static final Logger LOGGER = Logger.getLogger(SortEngine.class.getName());
    ...
    public SortEngine(OrderByNode orderBy, RowProjector projector) {
        ...
        if (hasSort()) {
            LOGGER.fine(() -> "ORDER BY loads all rows into memory; sort columns: "
                + orderBy.getItems());
        }
    }
    ...
    public List<Object[]> sort(List<Object[]> rows) {
        ...
        LOGGER.fine(() -> "Sorted " + rows.size() + " rows in " + elapsed + "ms");
