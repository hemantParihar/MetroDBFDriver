    ...
    public RowProjector(List<DBFField> fields, SelectNode selectNode) throws SQLException {
        ...
        if (selectNode == null) {
            ...
            for (ColumnNode col : selectNode.getColumns()) {
                ...
                if (col.isStar()) {
                    // * mixed with other select items - expand to all real columns
                    for (int i = 0; i < fields.size(); i++) {
                        selectedColumnIndexes.add(i);
                        String fieldName = fields.get(i).getName();
                        columnNames.add(fieldName);
                        columnLabels.add(fieldName);
                        columnNameToIndex.putIfAbsent(fieldName.toLowerCase(), columnPosition);
                        columnPosition++;
                    }

                // Check for RECNO() pseudo-column
                } else if (colName != null && ("RECNO".equalsIgnoreCase(colName) || "RECNO()".equalsIgnoreCase(colName))) {
                    // RECNO pseudo-column
