...
public class DBFResultSet implements ResultSet {
    ...
    // Row limit from SQL TOP n / LIMIT n (-1 = unlimited)
    private int limit = -1;

    // Decode mask for the plain reader path (null = decode every column)
    ...
    public DBFResultSet(DBFReader reader, SelectNode selectNode) throws SQLException {
        ...
        if (selectNode != null && selectNode.getWhere() != null) {
            this.filter = new FilterEngine(selectNode.getWhere().getCondition(),
                reader.getHeader().getFields());
        } else {
            this.filter = null;
        }

        // Column pruning (same rules as the streaming path): decode only the
        // selected + WHERE columns; null = SELECT * / unresolved -> decode all.
        this.neededColumns = com.dbf.jdbc.execution.streaming.ProjectedReaderStream
            .computeNeeded(reader.getHeader().getFields(), selectNode, projector);

        // Note: ORDER BY never reaches this constructor - DBFStatement routes
        // it through ExternalMergeSortOperator so memory stays bounded.
    }
    ...
    public DBFResultSet(RowStream stream, SelectNode selectNode) throws SQLException {
        ...
        this.streaming = true;
        this.filter = null;
        this.neededColumns = null; // upstream operators already pruned
        
        // Build projector from stream columns, using real field definitions
        // when the stream can provide them
        this.projector = new RowProjector(streamFields(stream), selectNode);
    ...
    private Object[] readFullRowFromReader() throws IOException, SQLException {
    ...
    public boolean next() throws SQLException {
        ...
        // Enforce a SQL TOP n / LIMIT n row cap
        if (limit >= 0 && currentRowNumber >= limit) {
            currentRow = null;
            hasMoreRows = false;
            return false;
        }
    ...
    /** Sets the maximum number of rows this result set will return (TOP/LIMIT). */
    ...
    private boolean nextFromReader() throws SQLException {
        try {
            while (hasMoreRows) {
                ...
                if (filter == null || filter.matches(fullRow, recnoValue)) {
 ...
 // In DBFResultSet.java, modify getValue() method:
    ...
    private Object getValue(int columnIndex) throws SQLException {
        ...
        // RECNO values are filled into the projected row by RowProjector,
    ...
    @Override public BigDecimal getBigDecimal(int col, int scale) throws SQLException {
        ...
        return BigDecimal.valueOf(n.doubleValue()).setScale(scale, java.math.RoundingMode.HALF_UP);
    }
    ...
    @Override public Object getObject(int col) throws SQLException {
        Object v = getValue(col);
        // Integer values surface as java.lang.Integer, not Long: the engine
        // computes integral results as Long internally (type-preserving
        // arithmetic), but callers written for typical JDBC drivers handle
        // Integer/Double and choke on Long ((Double) casts in the METRO app
        // threw ClassCastException). Narrow at the JDBC boundary; only values
        // too large for int remain Long (narrowing those would corrupt them).
    ...
    public ResultSetMetaData getMetaData() throws SQLException {
        if (reader != null) {
            ...
            List<DBFField> fields = streamFields(rowStream);
            String[] colNames = rowStream.getColumnNames();
            List<Integer> allIndexes = new ArrayList<>();
            for (int i = 0; i < colNames.length; i++) {
                allIndexes.add(i);
            }
            return new DBFResultSetMetaData(this, fields, allIndexes, Arrays.asList(colNames));
        }
        throw SQLExceptionFactory.create("No metadata available");
    }

    /**
     * Field definitions for a stream's columns: real ones when the stream is
     * field-aware, otherwise generic character fields.
     */
    private static List<DBFField> streamFields(RowStream stream) {
        if (stream instanceof com.dbf.jdbc.execution.streaming.FieldAwareStream) {
            List<DBFField> fields =
                ((com.dbf.jdbc.execution.streaming.FieldAwareStream) stream).getFields();
            if (fields != null && fields.size() == stream.getColumnNames().length) {
                return fields;
            }
        }
        List<DBFField> dummyFields = new ArrayList<>();
        for (String name : stream.getColumnNames()) {
    ...
    @Override public Clob getClob(int col) throws SQLException {
        // Memo columns are typed Types.CLOB; some clients (and CachedRowSet on
        // some JDKs) fetch them via getClob rather than getObject/getString.
        // Return a real Clob over the memo text instead of null.
        String s = getString(col);
        return s == null ? null : new javax.sql.rowset.serial.SerialClob(s.toCharArray());
    }
    @Override public Clob getClob(String label) throws SQLException { return getClob(findColumn(label)); }
