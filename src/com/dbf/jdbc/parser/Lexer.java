...
public class Lexer {
    ...
    private void tokenize() throws IOException {
        while (currentChar != -1) {
            ...
            // Numbers. A leading '-' is part of a negative literal only when a
            // value does NOT immediately precede it; after a value (number,
            // identifier, ')', string) a '-' is the binary subtraction operator,
            // so e.g. MAX(V)-1 and col-1 parse as subtraction, not "value, -1".
            if (Character.isDigit(currentChar)
                    || (currentChar == '-' && lookaheadIsDigit() && !previousIsValue())) {
                readNumber();
                continue;
            }
            ...
            // MS-Access date literal: #yyyy-mm-dd# (or #mm/dd/yyyy#)
            if (currentChar == '#') {
                readDateLiteral();
                continue;
            }

            // Quoted identifiers: `name` (MySQL) or [name] (MS Access / SQL Server).
            // Always an identifier, never a keyword.
            if (currentChar == '`') {
    ...
    /** True if the previously emitted token is a value (so a following '-' is binary minus). */
    private boolean previousIsValue() {
        if (tokens.isEmpty()) {
            return false;
        }
        TokenType t = tokens.get(tokens.size() - 1).getType();
        return t == TokenType.NUMBER || t == TokenType.IDENTIFIER
            || t == TokenType.STRING || t == TokenType.RPAREN || t == TokenType.RECNO;
    }
    ...
    /**
     * Reads a quoted identifier delimited by {@code close} (e.g. `name` or
     * [name]). The content is taken verbatim and emitted as an IDENTIFIER, so
     * reserved words and spaces inside the quotes are treated as a plain name.
     */
    private void readQuotedIdentifier(char close) throws IOException {
        int startLine = line;
        int startCol = column;
        advance(); // skip the opening quote
        StringBuilder sb = new StringBuilder();
        while (currentChar != -1 && currentChar != close) {
            sb.append((char) currentChar);
            advance();
        }
    ...
    /**
     * Reads a MS-Access date literal delimited by '#'. The value is emitted
     * as a STRING token normalised to ISO yyyy-MM-dd, which compares
     * correctly against java.sql.Date values (whose toString() is ISO).
     */
    private void readDateLiteral() throws IOException {
        int startLine = line;
        int startCol = column;
        StringBuilder sb = new StringBuilder();
        ...
        tokens.add(new Token(TokenType.STRING, normalizeDate(sb.toString().trim()),
            startLine, startCol));
    }

    /** Normalises common date formats to ISO yyyy-MM-dd; leaves others as-is. */
